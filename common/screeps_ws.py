...
class Client:
    def __init__(self, token: str = None, email: str = None, password: str = None,
                 ...
                 running_until_subscribe_is_empty: bool = True,

                 subscribeOnce: bool = True,
                 gzip: bool = False,
                 logging: bool = False,
    ...
    async def __runtime_onReceiveRawData(self, ws, rawdata):
        ...
        if response['data'] == {}:  # TODO =={}的情况有哪些？ 已知：未开放房间
    ...
    class roomMap:
        class RoomMapData:
            def __init__(self, event=None, data=None):
                ...
                self.wall_positions = data["w"]
                self.road_positions = data["r"]
                self.power_bank_positions = data["pb"]
                self.portal_positions = data["p"]
                self.source_positions = data["s"]
                self.source_count = len(self.source_positions)
                self.controller_position = data["c"]
                self.mineral_position = data["m"]
                self.source_keeper_positions = data["k"]

                self.player_objects_dict_id_positions = [{k: v} for k, v in data.items() if len(k) > 2]

        @staticmethod
        def getDataFromEvent(event):
            return defaultdict(list).update(event["data"])

        @staticmethod
        def getSourceCountFromData(data: dict):
            """
            you d better use data from 'getDataFromEvent' method, otherwise may cause KeyError
            :param data:
            :return:
            """
            try:
